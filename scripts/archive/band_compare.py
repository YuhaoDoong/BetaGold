"""Compare band configurations: test all upper/lower lag combinations."""
...
def main():
    ...
    band_configs = [
        ("U=Daily(1)  L=LagAvg(1,2,3)  [current]", (1,), (1, 2, 3)),
        ("U=Daily(1)  L=Daily(1)", (1,), (1,)),
        ("U=LagAvg(1,2,3)  L=LagAvg(1,2,3)", (1, 2, 3), (1, 2, 3)),
        ("U=LagAvg(1,2,3)  L=Daily(1)", (1, 2, 3), (1,)),
    ]

    print("\nBuilding signals for each band config...")
    all_panels = []
    for name, ul, ll in band_configs:
        ub, lb, bp = build_band(oos, close, upper_lags=ul, lower_lags=ll)
        ...
        all_panels.append((name, trades, ub, lb, ex))
    ...
    fig.suptitle('Band Configuration Comparison (RV 10d, Band+Pullback) '
                 '— 2025-09 ~ 2026-03',
    ...
    for (name, trades, ub, lb, ex), ax in zip(all_panels, axes):
    ...
    out_pdf = 'outputs/band_config_comparison.pdf'
    with PdfPages(out_pdf) as pdf:
        pdf.savefig(fig, dpi=150, bbox_inches='tight')
    out_png = 'outputs/band_config_comparison.png'
