...
**关键修正** (v2.0.1): 黄金是全球 24h 市场, 不能只看美股 GLD 的 7h 交易时段。
- **GC=F (COMEX期货) 为主信号源**: 23h/天, ~19根/天 (覆盖亚洲/伦敦/纽约全时段)
- **GLD 含盘前盘后**: 17根/天 (04:00~19:00 ET), 作为跨市场参考
- **Horizon 修正**: 19h≈1天, 95h≈5天 (基于 GC=F 的 24h 交易日, 非 GLD 的 7h)

**数据**:
- GC=F 1h: 11,422 根 (606天, 主信号源)
- GLD 1h: 8,289 根 (含盘前盘后)
- DXY/VIX/SLV/TLT 1h: 跨市场特征

**模型结果** (OOS 2025-09 ~ 2026-03, 2800+ 样本, q95/05 cal90):

| 指标 | 19h (1天) | 95h (5天) |
|------|---------|---------|
| Coverage (联合) | **76.2%** | 63.2% |
| Coverage (上界) | **96%** | **87%** |
| Coverage (下界) | **81%** | 74% |
| IC upper | 0.156 | -0.199 |
| IC lower | **0.212** | **0.354** |
| 区间宽度 | 5.41% | 9.62% |

上界覆盖 96%/87% → 退出信号高度可靠; 下界 81%/74% → 买入信号可用。

**设计理念**: 用 95h(5天) Band 确定买入/退出区间 (持仓 2-5 天, 适合期权),
用 1h 价格实时计算 bp → 盘中精确触发, 不需等收盘。

**2026 年回测** (95h Band + GC=F 1h bp, 持仓 2-10 天):

| 指标 | v1.0 日线 | v2.0.1 GC=F | 提升 |
|------|---------|------------|------|
| 交易数 | 2 笔 | **17 笔** | +750% |
| 胜率 | 50% | **76%** | +26pp |
| 均收益 | +0.88% | **+1.95%** | +121% |
| **累计收益** | **+1.75%** | **+37.55%** | **+21x** |
| 均持仓 | 5.0 天 | **2.7 天** | 更高效 |
| 最大盈利 | +2.03% | **+10.27%** | 1月底大涨 |
| 最大亏损 | -0.27% | -5.01% | 3月初回调 |

亮点: 1-2月金价从 $4400→$5400 的上涨行情, v2.0.1 捕捉到 12 笔盈利交易, v1.0 只有 1 笔。

### v2.1 (2026-03-17) — 正确架构: v1.0 Band + 盘中触发 + 1h确认

**教训**: v2.0 的 GC=F 1h 模型重新训练了 Band, 但只有 2 年牛市数据, 导致上界系统性偏高 (1月底高点 bp 只到 0.69, 触发不了退出)。v1.0 的 20年日线 Band 校准更准。

**正确做法**: 不替换 Band, 而是改善执行精度。

```
v1.0 Band (20年训练, 校准可靠) → 每天算出买入价/退出价
    ↓
GLD 盘中价 (含盘前盘后 04:00~19:00 ET, 17根/天) → 实时判断
GC=F 补充 19:00~04:00 盲区 → 换算 GLD 等价
    ↓
1h RSI/动量/止跌形态 → 入场确认 (方向A)
    ↓
信号输出: BUY/SELL/EXIT [+ 1h确认状态]
```

**三种模式对比** (2026年, 全部信号):

| 模式 | 买入天 | 退出天 | 买入准确(5天未跌>3%) | 退出准确(5天未涨>3%) |
|------|-------|-------|-------------------|-------------------|
| A: 日线收盘 | 4 | 3 | 3/3 (100%) | 2/3 (67%) |
| **B: 日线Band+盘中H/L** | **8** | **7** | **5/6 (83%)** | **5/7 (71%)** |
| C: B+1h确认 | 8 | 7 | 5/6 (83%) | 5/7 (71%) |

模式B 是当前最佳平衡: 信号翻倍 (4→8, 3→7), 准确率保持高位 (83%/71%)。1h确认 (模式C) 框架已就绪, 需要更多数据调优阈值。

### v2.2 (2026-03-17) — 12h 止盈 + 参数化

**多时间尺度止盈对比** (2025-09 ~ 2026-03, 134 交易日, 入场统一日线 H/L):

| 尺度 | 笔数 | 胜率 | 累计 | 最大亏 | Sharpe |
|------|------|------|------|--------|--------|
| 日线 | 10 | 90% | +20.3% | -5.03% | 0.55 |
| **12h** | **13** | **85%** | **+37.5%** | **-0.96%** | **0.78** |
| 8h | 13 | 85% | +33.7% | -1.50% | 0.64 |
| 4h | 13 | 85% | +35.9% | -1.50% | 0.70 |
| 2h | 13 | 77% | +38.2% | -0.94% | 0.75 |
| 1h | 13 | 85% | +30.6% | -1.35% | 0.77 |

12h 最优: Sharpe 最高 (0.78), 最大亏仅 -0.96%, 比日线快速止损 (1/29: -0.96% vs -5.03%), 比 1h 更有耐心让利润奔跑 (2/26: +10.3% vs +7.5%)。

**参数化**: 止盈尺度、Pullback 阈值、MACD 最低盈利等均可在 `core/signals_v2.py` 顶部配置。

**待办**:
- [ ] 方向B: 1h 特征辅助日线模型微调
- [ ] 在线 inference (实时预测不依赖 OOS parquet)
