...
- 50% profit lock 早平 (移动 > 1.5σ 即可)
- 实证胜率 79% (近 5y), 76% (近 8y) — 是有效策略

### IV Crush 风险 (v3.7.14, 仅作参考)

**实证 GLD/GVZ 数据 (2025-2026 FOMC, n=10):**

| 事件 | mean | median | 显著 crush 率 | SPX 对比 |
|------|------|--------|---------------|----------|
| FOMC | -0.1% | +0.1% | 40% (60% 反向) | -30~60% |
| NFP | -2.8% | -2.8% | 14% | -10~20% |
| OPEX | -5.4% | -0.6% | 0% | 不显著 |

**关键洞察**: 黄金对 FOMC 反应延迟 (要看实际利率/美元变化), IV 不会立即衰减.
**与 SPX/QQQ 完全不同**, 不要照搬 SPX 经验.

**Dashboard 实时显示 IV / RV 比率作为风险参考** (不调 P&L):
...
**默认配置**: `iv_crush_adj=False` — GLD 实证 crush 不显著, 不调 P&L.
若用 SPY/QQQ 等强 crush 资产, 可手动 `iv_crush_adj=True` 启用 5% 修正.

**Calendar Spread (替代方案)**: 当 IV/RV > 1.5 + 跨 FOMC 时考虑.
卖近月 + 买远月, 净 vega 微 long, 但近月对 IV 更敏感, IV crush 时净赚.
不是纯做空波动率, 是做空 IV 期限结构.
