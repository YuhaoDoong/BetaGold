...
# ── 期货 ──
# GLD 20× (XAUUSDT, 黄金日波 ~1-2%, max lev 20×)
# SLV 10× (XAGUSDT, 白银日波 ~3-5%, 同样 SL -50% margin 但 spot 阈值 2× 给波动留空间)
FUTURES_GLD = FuturesConfig(
    leverage=20,
    tp_margin_pct=200.0,    # +200% margin = +10% spot
    sl_margin_pct=50.0,     # -50% margin = -2.5% spot (先于爆仓 -100%/-5%)
    hold_max_days=15,
    early_tp_locks=(
        (3, 5.0),    # 3d ≥ +5% spot (+100% margin) 锁利
        (7, 3.0),    # 7d ≥ +3% spot (+60% margin)
        (10, 1.0),   # 10d ≥ +1% spot (+20% margin)
    ),
)
FUTURES_SLV = FuturesConfig(
    leverage=10,
    tp_margin_pct=200.0,    # +200% margin = +20% spot
    sl_margin_pct=50.0,     # -50% margin = -5% spot
    hold_max_days=15,
    early_tp_locks=(
        (3, 10.0),   # 3d ≥ +10% spot (+100% margin)
        (7, 6.0),    # 7d ≥ +6% spot (+60% margin)
        (10, 2.0),   # 10d ≥ +2% spot (+20% margin)
    ),
)
